...
/* =========================
   COLORES ARCOÍRIS PASTEL
========================= */
function generarArcoirisPastel(n) {
  const colores = [];
  if (!n || n <= 0) return colores;

  const saturation = 45; // pastel
  const lightness = 78;  // pastel

  for (let i = 0; i < n; i++) {
    const hue = Math.round((360 * i) / n); // reparte el arcoíris parejo
